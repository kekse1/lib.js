(function()
{
	...
	//
	const DEFAULT_DISTANCE_ALGORITHM = 'haversine'; //[ 'haversine', 'sphericalLawOfCosines' ];
	...
	//
	geo.distance = function(_lat1, _lon1, _lat2, _lon2, _geo_distance = DEFAULT_DISTANCE_ALGORITHM)
	{
		if(! (String.isString(_geo_distance) || Number.isInt(_geo_distance)))
		{
			_geo_distance = DEFAULT_DISTANCE_ALGORITHM;
		}

		if(Number.isInt(_geo_distance))
		{
			_geo_distance = geo.distances[geo.distances.getIndex(_geo_distance)];
		}
		
		if(String.isString(_geo_distance))
		{
			...
			return x('The % argument is not valid (see also \'%\')', null, '_geo_distance', 'geo.distances');
		}
		
		return x('No valid % argument (expecting a % or %, see also \'%\' and \'%\')', null, '_geo_distance', 'String', 'Integer', 'geo.distances', 'DEFAULT_DISTANCE_ALGORITHM');
	}
	...
	//
	geo.distance.haversine = function(_lat1, _lon1, _lat2, _lon2)
	{
		//
	...
	geo.distance.sphericalLawOfCosines = function(_lat1, _lon1, _lat2, _lon2)
	{
		//
